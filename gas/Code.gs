 ...
 *
 * 功能特色：
 * 1. 從 ORCID 抓取論文清單並比對 Google Sheet，自動新增未收錄之論文。
 * 2. 根據 DOI 從 Crossref 取得正式 BibTeX / JSON 並自動補齊各欄位。
 * 3. 從 Google Scholar 匯出之 BibTeX 連結補充未列於 ORCID 之論文。
 * 4. 支援 Web App URL 觸發（doGet）：透過瀏覽器打開或 HTTP GET 請求即可自動觸發同步。
 */

// --- 全域設定：請確認符合您的試算表與個人學術識別碼 ---
const SPREADSHEET_ID = "1EsbqSfOS97txN7_nwxpujdkV3g6scHB24TSAaJ25AMo";
const SHEET_NAME = "ORCID";
const ORCID_ID = '0000-0002-0426-9301'; 
const GOOGLE_SCHOLAR_BIBTEX_URL = "https://scholar.googleusercontent.com/citations?view_op=export_citations&user=pA0SI4cAAAAJ&citsig=ACUpqDcAAAAAaKpcPlTPShxhPdCIn9TtR6uDU08&hl=zh-TW";

// 安全權限金鑰（選填）：若設定字串，呼叫 URL 時需帶上 ?key=YOUR_SECRET_TOKEN 以防止未授權調用
// 若設為空字串 "" 則不限制金鑰
const ACCESS_TOKEN = "";

// 要從 BibTeX/JSON 解析並填入的欄位名稱
const BIBTEX_MAPPING = {
  'Authors': 'author',
  'Year': 'year',
  'Month': 'month', 
  'Volume': 'volume',
...
/**
 * Web App HTTP GET 入口點：支援透過 URL 讀取與觸發
 *
 * 呼叫範例：
 * 1. 預設完整同步（ORCID + Scholar + BibTeX）：
 *    https://script.google.com/macros/s/{DEPLOYMENT_ID}/exec
 * 2. 指定單一動作：
 *    https://script.google.com/macros/s/{DEPLOYMENT_ID}/exec?action=orcid
 *    https://script.google.com/macros/s/{DEPLOYMENT_ID}/exec?action=scholar
 *    https://script.google.com/macros/s/{DEPLOYMENT_ID}/exec?action=bibtex
 */
function doGet(e) {
  ...
  // 驗證存取金鑰（若有設定）
  if (ACCESS_TOKEN && ACCESS_TOKEN.trim() !== '' && key !== ACCESS_TOKEN) {
  ...
  try {
    if (action === 'orcid' || action === 'all') {
      responsePayload.results.orcid = updateMyPaperList();
    }
    if (action === 'scholar' || action === 'all') {
      responsePayload.results.scholar = checkByGoogleScholarBibtex();
    }
    if (action === 'bibtex' || action === 'all') {
      responsePayload.results.bibtex = checkAndUpdateBibtex();
    }

    responsePayload.durationSeconds = ((new Date().getTime() - startTime.getTime()) / 1000).toFixed(2);
...
function updateMyPaperList() {
  const result = { newPapers: 0, updatedDoi: 0, message: '' };
  try {
    ...
    result.message = `ORCID 同步完成：新增 ${result.newPapers} 篇，更新 DOI ${result.updatedDoi} 篇。`;
    return result;
...
function checkAndUpdateBibtex() {
  const result = { updatedCount: 0, message: '' };
  try {
    const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_NAME);
    if (!sheet) throw new Error(`找不到名為 "${SHEET_NAME}" 的工作表。`);
    ...
    if (updatedRows.length > 0) {
      ...
      result.updatedCount = updatedRows.length;
      result.message = `更新完成！共有 ${updatedRows.length} 篇論文的 BibTeX 資訊被更新。`;
      Logger.log(result.message);
    } else {
      result.message = '沒有找到任何需要更新的 BibTeX 資料。';
      Logger.log(result.message);
    }
    return result;
...
function checkByGoogleScholarBibtex() {
  const result = { newPapers: 0, message: '' };
  if (!GOOGLE_SCHOLAR_BIBTEX_URL || GOOGLE_SCHOLAR_BIBTEX_URL.length < 50) {
    result.message = '請先更新腳本中的 Google 學術 BibTeX 連結。';
    Logger.log(result.message);
    return result;
  }
  ...
  try {
    ...
    if (newWorks.length > 0) {
      sheet.getRange(sheet.getLastRow() + 1, 1, newWorks.length, newWorks[0].length).setValues(newWorks);
      result.newPapers = newWorks.length;
      result.message = `已成功從 Google 學術新增 ${newWorks.length} 篇論文。`;
      Logger.log(result.message);
    } else {
      result.message = '沒有從 Google 學術連結中找到新的論文。';
      Logger.log(result.message);
    }
    return result;
